...
class GoogleLoginRequest(BaseModel):
    ...
    `id_token` is the raw JWT credential Google Identity Services hands
    back to the frontend after the user picks their Google account. The
    backend verifies it server-side (signature, audience, expiry)
    against Google's own public keys before trusting anything in it -
    the frontend never gets to just assert who the user is.
    """
...
class UserResponse(BaseModel):
    ...
    auth_provider: str = Field(
        default="password", description="'password' or 'google' - how this account signs in"
    )
    created_at: datetime
    updated_at: datetime
