...
Real dashboard authentication endpoints (Phase 7 addition):
    ...
    POST /auth/google           - verify a Google ID token, returns a JWT
...
async def google_login(
    payload: GoogleLoginRequest, db: AsyncIOMotorDatabase = Depends(get_database)
):
    """
    Verify a Google Identity Services ID token server-side and log the
    user in, creating an account on first sign-in. Returns 503 if
    GOOGLE_CLIENT_ID is not configured, 401 if the token is invalid,
